...
import { Phone, Sparkles, Send } from "lucide-react";

interface HospitalityMobileStickyBarProps {
  onOpenAuditModal: () => void;
}

export default function HospitalityMobileStickyBar({
  onOpenAuditModal,
}: HospitalityMobileStickyBarProps) {
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 p-3 bg-white/95 backdrop-blur-xl border-t border-slate-200 shadow-[0_-8px_30px_rgba(0,0,0,0.12)] md:hidden">
      <div className="flex items-center gap-2">
        <a
          href="https://wa.me/2349067623555?text=Hello%20ChyWeb%20Academy%2C%20I%20want%20to%20claim%20my%20Free%20Hospitality%20Audit."
          target="_blank"
          ...
          aria-label="WhatsApp Strategy Desk"
        >
          <Phone size={20} />
        </a>
        ...
        >
          <div className="text-left leading-tight">
            ...
            <span className="text-xs font-black block text-white">Turn Rooms Into Direct Bookings</span>
          </div>
          <Send size={15} className="text-white flex-shrink-0 ml-2" />
        </button>
      </div>
    </div>
  );
